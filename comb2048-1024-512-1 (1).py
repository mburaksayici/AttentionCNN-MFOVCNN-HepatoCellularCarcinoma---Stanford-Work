...
for i in range(5):
    ...
    class ImageFilelist(Dataset):
        ...
        def __getitem__(self, index):
            im2048, im1024, im512, target = self.listtrain2048[index], self.listtrain1024[index], self.listtrain512[index], self.listtrainlabel[index]
    ...
    class MyEnsemble(nn.Module):
        def __init__(self):
            ...
            self.modelSmall = nn.Sequential(*listofsmallwithoutfcnn)

            listofmediumwithoutfcnn = list(self.modelMedium.children())[:-1]
            self.modelMedium = nn.Sequential(*listofmediumwithoutfcnn)

            listoflargewithoutfcnn = list(self.modelLarge.children())[:-1]
            self.modelLarge = nn.Sequential(*listoflargewithoutfcnn)
        ...
        def forward(self, LargeImage, MediumImage, SmallImage):
            ...
            outMedium = self.modelMedium(LargeImage)
            outSmall = self.modelSmall(SmallImage)

            stacked = torch.stack((outLarge, outMedium, outSmall), dim=1)  # torch.cat((outlarge, outsmall), dim=1)

            stacked = stacked.view(-1, 2048 * 3)
            pred = self.classifier(stacked)
            return pred
    ...
    def set_parameter_requires_grad(model, feature_extracting):
        ...
        if feature_extracting:
            ...
            for name, param in model.named_parameters():
                ...
                if name == "modelSmall.7.2.bn3.bias":
                    a = 1
